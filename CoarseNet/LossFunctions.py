...
from tensorflow.keras.layers import Activation, AveragePooling2D, BatchNormalization, Concatenate, Conv2D, Dense, GlobalAveragePooling2D, Input, Lambda, MaxPooling2D
...
def orientation_loss(y_true, y_pred, lamb=1.):
    ...
    coherence = K.sqrt(K.square(sin2angle) +
                       K.square(cos2angle)) / (modulus + K.epsilon())
    coherenceloss = K.sum(label_seg) / \
        (K.sum(coherence*label_seg) + K.epsilon()) - 1
...
def minutiae_score_loss(y_true, y_pred):
    ...
    y_true = K.tf.where(K.tf.less(y_true, 0.0),
                        K.tf.zeros_like(y_true), y_true)  # set -1 -> 0
    # weighted cross entropy loss
