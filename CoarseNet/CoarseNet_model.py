...
def CoarseNetmodel(input_shape=(400, 400, 1), weights_path=None, mode='train'):
    ...
    # sum fusion for ori
    ori_out = Lambda(CoarseNet_utils.merge_sum)([ori_1, ori_2, ori_3])
    ...
    seg_out = Lambda(CoarseNet_utils.merge_sum)([seg_1, seg_2, seg_3])
    ...
    mnt_o_1 = Lambda(CoarseNet_utils.merge_concat)([mnt_conv, ori_out_1])
...
#             mnt = label2mnt(mnt_s_out, mnt_w_out, mnt_h_out,
#                             mnt_o_out, thresh=early_minutiae_thres)
#             # Previous exp: 0.2
...
#         image = misc.imread(deploy_set + 'img_files/' +
#                             img_name[i] + '.bmp', mode='L')  # / 255.0
#         mask = misc.imread(deploy_set + 'seg_files/' +
#                            img_name[i] + '.bmp', mode='L') / 255.0
...
#         time_afterpost = time()
#         mnt_writer(mnt_nms, img_name[i], img_size, "%s/%s/mnt_results/%s.mnt" %
#                    (output_dir, set_name, img_name[i]))
#         draw_minutiae_overlay_with_score(
#             image, mnt_nms, mnt_gt[:, :3], "%s/%s/%s_minu.jpg" % (output_dir, set_name, img_name[i]), saveimage=True)
#         # misc.imsave("%s/%s/%s_score.jpg"%(output_dir, set_name, img_name[i]), np.squeeze(mnt_s_out_upscale))

#         misc.imsave("%s/%s/seg_results/%s_seg.jpg" %
...
#         image = misc.imread(deploy_set + 'img_files/' +
#                             img_name[i] + file_ext, mode='L')  # / 255.0
...
#             mnt_nms_1 = mnt_nms_1_copy[mnt_nms_1_copy[:, 3]
#                                        > early_minutiae_thres, :]
#             mnt_nms_2 = mnt_nms_2_copy[mnt_nms_2_copy[:, 3]
#                                        > early_minutiae_thres, :]
...
#         mnt_writer(mnt_nms, img_name[i], img_size, "%s/%s/mnt_results/%s.mnt" %
#                    (output_dir, set_name, img_name[i]))
#         draw_minutiae(original_image, mnt_nms, "%s/%s/%s_minu.jpg" %
#                       (output_dir, set_name, img_name[i]), saveimage=True)
